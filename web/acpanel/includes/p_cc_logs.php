...
if( !isset($_GET['server']) )
{
	...
	$sub = $db->Query("SELECT b.id, b.hostname, b.address FROM `acp_chat_logs` a 
...
{
	$go_page = "p_cc_logs_result";

	$srv = trim($_GET['server']);
	$type = $_GET['msgtype'];
	$status = $_GET['msgstatus'];
	...
	if ($type != "all") { $postout .= "&type=".$type; $sqlconds .= " AND cmd = '{cmd}' "; }
	if ($status != "all") { $postout .= "&status=".$status; $sqlconds .= " AND pattern = '{status}' "; }
	...
	if ($msg) { $postout .= "&msg=".$msg; $sqlconds .= " AND message LIKE '%{message}%' "; }

	date_default_timezone_set('UTC');
	$arguments = array('status'=>$status,'serverip'=>$srv,'cmd'=>$type,'name'=>$pl_nick,'authid'=>$pl_id,'ip'=>$pl_ip,'startdate'=>get_datetime(strtotime($startdate), false, true),'enddate'=>get_datetime(strtotime($enddate), false, true),'message'=>$msg);
	$total_items = $db->Query("SELECT count(*) FROM `acp_chat_logs` $sqlconds", $arguments, $config['sql_debug']);
