...
{
	$search_addcat_id = array_search("p_gamebans_subnets_add", $value);
	$search_editcat_id = array_search("p_gamebans_subnets_edit", $value);
	if( $search_addcat_id )
	{
		$search_addcat = $key;
...
if( $t )
{
	$total_items = $db->Query("SELECT count(*) FROM `acp_bans_subnets` WHERE approved = ".(($t == 1) ? 1 : 0), array(), $config['sql_debug']);
}
else
{
	$total_items = $db->Query("SELECT count(*) FROM `acp_bans_subnets`", array(), $config['sql_debug']);
}
	...
	<script type='text/javascript'>
		...
		function pageselectCallback(page_id, total, jq) {
			...
			jQuery.ajax({
				...
				data:'go=11&status=' + status + '&cat_edit=' + cat_edit + '&cat_current=' + cat_current + '&offset=' + first + '&limit=' + pg_size,
