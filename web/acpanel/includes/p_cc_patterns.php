...
if( !isset($_GET['s']) || !is_numeric($_GET['s']) || !in_array($_GET['s'], array('0','1','2','3','4')) ) {
...
$total_items = $db->Query("SELECT count(*) FROM `acp_chat_patterns` WHERE action = '{action}'", $arguments, $config['sql_debug']);
...
{
	$search = array_search("p_cc_patterns_add", $value);
	if ($search)
	{
