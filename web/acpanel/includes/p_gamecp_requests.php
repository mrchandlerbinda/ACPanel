...
if( !isset($_GET['s']) || !is_numeric($_GET['s']) || !in_array($_GET['s'], array('0','1','2','3')) ) {
...
{
	$search_edit = array_search("p_gamecp_requests_edit", $value);
	if( $search_edit )
	{
		$search_edit_id = $key;
		break;
...
$total_items = $db->Query("SELECT count(*) FROM `acp_players_requests`".$sqlconds, $arguments, $config['sql_debug']);
	...
	<script type='text/javascript'>
		...
		function pageselectCallback(page_id, total, jq) {
			...
			jQuery.ajax({
				...
				data:'go=19&username=".$userinfo['username']."&edit_id=' + edit_id + '&cat_current=' + cat_current + '&offset=' + first + '&limit=' + pg_size + '&s=".$s."',
