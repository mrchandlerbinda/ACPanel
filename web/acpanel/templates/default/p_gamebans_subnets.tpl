<div class="block">
	<div class="block_head">
		...
		<h2>@@gamebans_subnets@@</h2>

		<ul id="select-list">
			{if $cat_addcat_id}
				<li><a href="{$home}?cat={$section_current.id}&do={$cat_addcat_id}" rel="facebox">@@add_ban_subnet@@</a></li>
			{/if}
			<li>
				<form id="forma-select" action="" method="get">
					<select class="styled" name="t">
						...
						<option value="1"{if $get_status == 1} selected{/if}>@@status_active@@</option>
						<option value="2"{if $get_status == 2} selected{/if}>@@status_inactive@@</option>
					</select>
				</form>
			</li>
		</ul>
